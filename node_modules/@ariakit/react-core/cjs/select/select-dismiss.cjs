"use strict";Object.defineProperty(exports, "__esModule", {value: true});"use client";


var _E3PQD36Dcjs = require('../__chunks/E3PQD36D.cjs');
require('../__chunks/GQVDSP7N.cjs');
require('../__chunks/X5YEYHCO.cjs');


var _RBHGB7EBcjs = require('../__chunks/RBHGB7EB.cjs');
require('../__chunks/RGJOOXFU.cjs');
require('../__chunks/3MQN464L.cjs');
require('../__chunks/32EK4BGY.cjs');
require('../__chunks/AMARRBCF.cjs');
require('../__chunks/HHTQYHYL.cjs');
require('../__chunks/7K2RM2SD.cjs');
require('../__chunks/46DIVJOT.cjs');
require('../__chunks/DYIDES27.cjs');




var _M5IWBORBcjs = require('../__chunks/M5IWBORB.cjs');
require('../__chunks/WT6AUJWV.cjs');
require('../__chunks/54XFMNDS.cjs');



var _3IVNQNIVcjs = require('../__chunks/3IVNQNIV.cjs');

// src/select/select-dismiss.tsx
var TagName = "button";
var useSelectDismiss = _M5IWBORBcjs.createHook.call(void 0, 
  function useSelectDismiss2(_a) {
    var _b = _a, { store } = _b, props = _3IVNQNIVcjs.__objRest.call(void 0, _b, ["store"]);
    const context = _RBHGB7EBcjs.useSelectScopedContext.call(void 0, );
    store = store || context;
    props = _E3PQD36Dcjs.usePopoverDismiss.call(void 0, _3IVNQNIVcjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var SelectDismiss = _M5IWBORBcjs.forwardRef.call(void 0, function SelectDismiss2(props) {
  const htmlProps = useSelectDismiss(props);
  return _M5IWBORBcjs.createElement.call(void 0, TagName, htmlProps);
});



exports.SelectDismiss = SelectDismiss; exports.useSelectDismiss = useSelectDismiss;
